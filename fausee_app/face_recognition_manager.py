...
class FaceRecognitionManager:
    ...
    def _wnd_proc(self, hwnd, msg, wparam, lparam):
        if msg == WM_WTSSESSION_CHANGE:
            if wparam == WTS_SESSION_LOCK:
                self.logger.system_locked()
                self.pause_recognition.set()
            elif wparam == WTS_SESSION_UNLOCK:
                self.logger.system_unlocked()
                self.pause_recognition.clear()
        return win32gui.DefWindowProc(hwnd, msg, wparam, lparam)
    ...
    @staticmethod
    def _probe_camera(device_index=0):
    ...
    def wait_for_camera(self):
        if self._probe_camera():
            if self._cam_down:
                self.logger.camera_accessible()
                self._cam_down = False
            return True

        if not self._cam_down:
            self.logger.camera_inaccessible()
            self._cam_down = True

        while not self._probe_camera():
            time.sleep(CAMERA_RETRY_DELAY)
