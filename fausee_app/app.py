...
class MonitorAppController:
    ...
    def __init__(self):
        ...
        self.db_manager = DBManager()
        self.analyzer = LogAnalyzer(self.logger_manager.get_log_dir(), self.db_manager)
        image_dir = os.path.join(os.getenv('APPDATA'), 'FaceVerificationApp', 'Images') # User-space directory
        self.face_manager = FaceRecognitionManager(self.logger_manager, image_dir=image_dir)
    ...
    def start_recognition_loop(self, parent_window=None):
        ...
        if ref_embed is None:
            ...
            ref_embed = self.face_manager.ensure_reference_embedding()
            if ref_embed is None:
                self.logger_manager.log_event("Reference image capture cancelled. Not starting.", level="warning")
                return

        if self.recognition_thread and self.recognition_thread.is_alive():
            self.face_manager.pause_recognition.clear()
            if not self.monitoring_active:
                self.logger_manager.monitoring_started()
            self.monitoring_active = True
            return
    ...
    def stop_recognition(self):
        if self.monitoring_active:
            self.logger_manager.monitoring_stopped()
        self.face_manager.pause_recognition.set()
        self.monitoring_active = False
...
def run_app():
    controller = MonitorAppController()
    controller.logger_manager.start_session()
    ...
    app = DashboardApp(controller)
    try:
        app.protocol("WM_DELETE_WINDOW", lambda: (controller.logger_manager.stop_session(), app.destroy()))
        app.mainloop()
    finally:
        if controller.monitoring_active:
            controller.logger_manager.monitoring_stopped()
